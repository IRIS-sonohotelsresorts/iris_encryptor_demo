...
public class KISA_SEED_ECB {
    ...
    public static class Common {
        ...
        public static String EncSeedEcb(String originData, String encKey) throws UnsupportedEncodingException {
            // User secret key
            ...
            byte pbUserKey[] = encKey.getBytes();
            byte[] pbData2    = null;
            //try {
            pbData2 = originData.getBytes("UTF-8");
            //} catch (UnsupportedEncodingException e1) {
            //	e1.printStackTrace();
            //}
            System.out.println("getbytes result length = "+Integer.toString(pbData2.length));

            int PLAINTEXT_LENGTH = pbData2.length;
            int i;
            int[] data;
            byte[] cdata;
            int nRetOutLeng[] = new int[] { 0 };
            ...
            System.out.println("nPlainTextPadding="+Integer.toString(nPlainTextPadding));
            int CIPHERTEXT_LENGTH = PLAINTEXT_LENGTH + nPlainTextPadding;
            System.out.println("CIPHERTEXT_LENGTH="+Integer.toString(CIPHERTEXT_LENGTH));
            ...
            System.out.println("pbszCipherText->"+Integer.toString(pbszCipherText.length));
            System.out.print("Ciphertext(enc)\t: ");

            for(i=0; i<CIPHERTEXT_LENGTH; i++)	 	System.out.print(Integer.toHexString(0xff&pbszCipherText[i])+" ");
            System.out.print("\n\n");
            return  Base64.encodeBase64String(pbszCipherText);
        }


        public static String DecSeedEcb(String encStr, String decKey) {

            // User secret key
			/*byte pbUserKey[]  = {(byte)0x88, (byte)0xE3, (byte)0x4F, (byte)0x8F, (byte)0x08, (byte)0x17, (byte)0x79, (byte)0xF1,
			                     (byte)0xE9, (byte)0xF3, (byte)0x94, (byte)0x37, (byte)0x0A, (byte)0xD4, (byte)0x05, (byte)0x89}; */

            byte pbUserKey[]  = decKey.getBytes();

            byte[] pbszCipherText  = Base64.decodeBase64(encStr);
            //byte[] pbszCipherText    = new byte[encStr.length()];
            System.out.println("enc ed Str = "+Integer.toString(encStr.length()));

            //Base64.decodeBase64(base64String)
            //try {
            //pbszCipherText = encStr.getBytes();
				/*for(int i=0; i<pbszCipherText.length; i++){
					if(i < encStr.length()){
						pbszCipherText[i] = (byte)encStr.charAt(i);
					}
				}*/
            ...
            byte[] pbszPlainText = new byte[process_blockLeng];
            int[] outbuf = new int[process_blockLeng];
            int CIPHERTEXT_LENGTH = pbszCipherText.length;
            System.out.println("CIPHERTEXT_LENGTH="+Integer.toString(CIPHERTEXT_LENGTH));
            ...
            System.out.print("PLAINTEXT_LENGTH : "+Integer.toString(PLAINTEXT_LENGTH));
            System.out.print("Plaintext(dec)\t: ");
            String m = "";
            try {
                m = new String(result, "UTF-8");
            } catch (UnsupportedEncodingException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            }
            System.out.println("dec result string ->"+m);
